...
fin.desktop.main(async () => {
  ...
  function notificationCleared(
      payload: {id: string}&ISenderInfo, sender: ISenderInfo) {
    // For testing/display purposes
  ...
  fin.notifications = {
    ...
    buttonClickHandler:
        async (payload: Notification&ISenderInfo, buttonIndex: number) => {
          ...
          const success =
              await plugin.dispatch('notification-button-clicked', fullPayload);
          console.log('success', success);
        },
    ...
    fetchAppNotifications: async (uuid: string) => {
      ...
      const appNotifications =
          await plugin.dispatch('fetch-app-notifications', payload);
    ...
    addEventListener: async (
        event: string,
        cb: (payload: Notification|ISenderInfo, sender: ISenderInfo) =>
            string) => {
  ...
  plugin.register(
      'app-notifications-cleared',
      (payload: ISenderInfo, sender: ISenderInfo) =>
          callbacks.appNotificationsCleared(payload, sender));
  plugin.register('all-notifications-cleared', allNotificationsCleared);
});

function allNotificationsCleared(
    payload: {id: string}&ISenderInfo, sender: ISenderInfo) {
  // Should remove the notification from the store/from the DOM
  fin.desktop.InterApplicationBus.publish(
      'ui-all-notifications-clear', payload);
