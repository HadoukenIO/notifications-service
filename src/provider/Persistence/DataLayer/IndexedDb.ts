...
export class IndexedDb<T extends Entity> implements IDatastore<T> {
   ...
   */
  public create<T extends Entity>(tableName: string, entry: T):
      Promise<boolean> {
    if (!tableName) {
   ...
   * @returns {Promise<boolean>} A value of whether it was successfully created or not
   */
  public remove<T extends string|number>(tableName: string, id: T):
      Promise<boolean> {
    if (!tableName) {
      console.error('No table name has been passed');
      return Promise.reject('No table name has been passed');
   ...
   * @returns {Promise<boolean>} A value of whether it was successfully created or not
   */
  public update<T extends Entity>(tableName: string, entry: T):
      Promise<boolean> {
    if (!tableName) {
      console.error('No table name has been passed');
      return Promise.reject('No table name has been passed');
  ...
  public readAll(tableName: string): Promise<T[]> {
    return new Promise((resolve, reject) => {
      ...
      cursorRequest.onsuccess = (event: Event) => {
        const cursor =
            (event.target as IDBRequest).result as IDBCursorWithValue;
        if (cursor) {
      ...
      this.readAll(tableName).then((notifications: T[]) => {
        if (notifications.length === 0) {
          ...
          if (pageInfo.sort === Sorts.ascending) {
            ...
            for (let i = reverseLoopOffset;
                 i > reverseLoopOffset - pageInfo.numberOfItems; i--) {
              if (notifications[i] != null) {
                result.push(notifications[i]);
              }
            }
