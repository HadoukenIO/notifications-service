...
export class HistoryRepository extends Repository<DataType> {
  ...
  public async create(notification: Notification&
                      ISenderInfo): Promise<ReturnResult<DataType>> {
  ...
  public async removeByUuid(uuid: string): Promise<VoidResult> {
    ...
    if (!result) {
      return {
        success: result,
        errorMsg:
            'Could not remove all entries in the database with the given uuid: ' +
            uuid
      };
    }
   ...
   * @returns {Promise<ReturnResult>} Success message and value return back to calling client
   */
  public async update(updatedNotification: Notification&
                      ISenderInfo): Promise<ReturnResult<DataType>> {
   ...
   * @returns {Promise<ReturnResult>} Success message and value return back to calling client
   */
  public async getByPage(pageInfo: PageInfo):
      Promise<ReturnResult<DataType[]>> {
