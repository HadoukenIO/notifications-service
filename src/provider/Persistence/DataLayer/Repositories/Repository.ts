...
export abstract class Repository<T extends Entity> {
  ...
  protected async genericGetAll(): Promise<ReturnResult<T[]>> {
    ...
    if (result == null) {
      return {
        success: false,
        errorMsg:
            `Could not retrieve all entities from the table ${this.TABLENAME}`,
        value: null
      };
    }
  ...
  protected async genericUpdate(entity: T): Promise<ReturnResult<T>> {
    ...
    if (!read.success) {
      return {
        ...
        errorMsg: `No entry matching the id: ${
            entity.id}, so there is no entry to be updated`,
        value: null
      };
    }
   ...
   * @returns {Promise<ReturnResult>} Success message and value return back to calling client
   */
  protected async genericGetByPage(pageInfo: PageInfo):
