...
export class SubscribeFeed extends Action<RootState> {
    public readonly type = '@@NOTIFICATIONS/SUBSCRIBE_FEED';
    ...
    public reduce(state: RootState): RootState {
        ...
        const subscribedFeed = feeds.find((candidate) => candidate.id === this.feedId);
        if (subscribedFeed) {
...
export class UnsubscribeFeed extends Action<RootState> {
    public readonly type = '@@NOTIFICATIONS/UNSUBSCRIBE_FEED';
    public readonly feedId: string;
    ...
    public reduce(state: RootState): RootState {
        ...
        const subscribedFeed = feeds.find((candidate) => candidate.id === this.feedId);
        if (subscribedFeed) {
            subscribedFeed.subscribed = false;
        }
        return {
            ...state,
            feeds: [
