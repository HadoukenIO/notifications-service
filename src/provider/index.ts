...
const historyRepository =
    repositoryFactory.getRepository(Repositories.history) as HistoryRepository;
const settingsRepository = repositoryFactory.getRepository(
                               Repositories.settings) as SettingsRepository;
...
async function registerService() {
  ...
  providerChannel.register(
      'notification-button-clicked', notificationButtonClicked);
  ...
  window.fin.notifications = {
    notificationCreated: async (payload: Notification&ISenderInfo) => {
      ...
      const success = await providerChannelPlugin.dispatch(
          serviceSenderInfo, 'notification-created', payload);
    ...
    notificationCleared: async (payload: {id: string}&ISenderInfo) => {
      ...
      const providerChannelPlugin = await providerChannel;
      const success = await providerChannelPlugin.dispatch(
          serviceSenderInfo, 'notification-cleared', payload);
      console.log('success', success);
    },
    appNotificationsCleared: async (payload: {uuid: string}) => {
      ...
      const providerChannelPlugin = await providerChannel;
      const success = await providerChannelPlugin.dispatch(
          serviceSenderInfo, 'app-notifications-cleared', payload);
      console.log('success', success);
    },
    notificationClicked: async (payload: NotificationEvent) => {
      ...
      const providerChannelPlugin = await providerChannel;
      const success = await providerChannelPlugin.dispatch(
          {name: payload.name, uuid: payload.uuid}, 'notification-clicked',
          payload);
      console.log('success', success);
    },
    notificationButtonClicked:
        async (payload: Notification&ISenderInfo&{buttonIndex: number}) => {
          // Send notification clicked to the Client
          const providerChannelPlugin = await providerChannel;
          const success = await providerChannelPlugin.dispatch(
              {name: payload.name, uuid: payload.uuid},
              'notification-button-clicked', payload);
          console.log('success', success);
        },
    notificationClosed: async (payload: NotificationEvent) => {
      ...
      const providerChannelPlugin = await providerChannel;
      const success = await providerChannelPlugin.dispatch(
          {name: payload.name, uuid: payload.uuid}, 'notification-closed',
          payload);
      console.log('success', success);
    },
    allNotificationsCleared: async (payload: NotificationEvent) => {
      const providerChannelPlugin = await providerChannel;
      const success = await providerChannelPlugin.dispatch(
          serviceSenderInfo, 'all-notifications-cleared', payload);
      console.log('success', success);
    }
  };
}
 ...
 */
function encodeID(payload: Notification&ISenderInfo|NotificationEvent|
                  {id: string}&ISenderInfo): string {
 ...
 * @param payload The notification object
 */
function decodeID(payload: Notification&ISenderInfo|NotificationEvent|
                  {id: string}&ISenderInfo): string {
...
async function createNotification(payload: Notification, sender: ISenderInfo) {
  ...
  const fullPayload: Notification&ISenderInfo =
      Object.assign({}, payload, sender, {type: noteType});
  const encodedID: string = encodeID(fullPayload);

  const fullPayloadEncoded: Notification&ISenderInfo =
      Object.assign({}, fullPayload, {id: encodedID});
...
async function clearNotification(payload: Notification, sender: ISenderInfo) {
  ...
  const fullPayload: {id: string}&ISenderInfo =
      Object.assign({}, payload, sender);
  const encodedID: string = encodeID(fullPayload);
...
function notificationClicked(
    payload: Notification&ISenderInfo, sender: ISenderInfo) {
  // For testing/display purposes
...
function notificationButtonClicked(
    payload: Notification&ISenderInfo&{buttonIndex: number},
    sender: ISenderInfo) {
  // For testing/display purposes
 ...
 * @param {object} sender UI Window info. Not important.
 */
async function notificationClosed(
...
async function clearAllNotifications(
    payload: Notification, sender: ISenderInfo) {
...
function testDisplay(
    action: string, payload: Notification|NotificationEvent|{uuid: string},
    sender: ISenderInfo) {
