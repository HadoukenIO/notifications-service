...
fin.desktop.main(async () => {
  ...
  for (let index = 1; index < 7; index++) {
    ...
    document.getElementById(`clearbutton${index}`)
        .addEventListener('click', () => {
          clearNote(`1q2w3e4r${index}`);
        });
  }

  document.getElementById(`fetchAppNotifications`)
      .addEventListener('click', () => {
        getNotes().then((notifications) => {
          ...
          logit(`${
              notifications.value
                  .length} notifications received from the Notification Center`);
        });
      });

  ofnotes.addEventListener(
      'click', (payload: NotificationEvent, sender: ISenderInfo) => {
  ...
  ofnotes.addEventListener(
      'close', (payload: NotificationEvent, sender: ISenderInfo) => {
  ...
  ofnotes.addEventListener(
      'button-click', (payload: NotificationEvent, sender: ISenderInfo) => {
